      ...
      <div className="border-b border-border bg-panel">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 font-mono text-xs text-text-muted">
            <Link to="/agents" className="hover:text-text-secondary transition-colors">AGENTS</Link>
            <span>/</span>
            ...
            <div className="animate-fade-in">
              ...
              >
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                  <path d="M8 2L4 6L8 10" stroke="currentColor" strokeWidth="1.5" />
                </svg>
                AGENTS
              </Link>
              ...
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <h1 className="font-mono text-lg tracking-tight text-text-primary">{agent.name}</h1>
                  <span className={`font-mono text-[10px] tracking-wide ${status.text}`}>
                    {status.label}
                  </span>
                </div>
                <p className="font-mono text-[11px] text-text-muted pl-5">{agent.agent_id}</p>
              </div>

              {/* Details grid */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8 stagger-children">
                <div className="border border-border bg-panel p-4">
                  <div className="font-mono text-[10px] tracking-widest text-text-muted mb-1">REGISTERED</div>
                  <div className="text-sm text-text-primary">{new Date(agent.registered_at).toLocaleDateString()}</div>
                </div>
                <div className="border border-border bg-panel p-4">
                  <div className="font-mono text-[10px] tracking-widest text-text-muted mb-1">ACTIVE GRANTS</div>
                  <div className="text-sm text-text-primary font-mono">{agent.grants.filter(g => g.status === 'active').length}</div>
                </div>
                <div className="border border-border bg-panel p-4">
                  <div className="font-mono text-[10px] tracking-widest text-text-muted mb-1">PUBLIC KEY</div>
                  <div className="text-sm text-text-primary font-mono truncate">{agent.public_key.slice(0, 24)}...</div>
                </div>
              </div>

              {/* Grants */}
              <div className="mb-8">
                <SectionLabel>GRANTS ({agent.grants.length})</SectionLabel>
                {agent.grants.length === 0 ? (
                  <div className="border border-border bg-panel p-6 text-center">
                    <p className="text-text-muted text-sm font-mono">NO ACTIVE GRANTS</p>
                  </div>
                ) : (
              ...
              {/* Activity */}
              <div className="mb-8">
                <SectionLabel>RECENT ACTIVITY ({events.length})</SectionLabel>
                {events.length === 0 ? (
                  <div className="border border-border bg-panel p-6 text-center">
                    <p className="text-text-muted text-sm font-mono">NO RECENT ACTIVITY</p>
                  </div>
                ) : (
              ...
              {agent.status === 'active' && (
                <div className="border border-red-dim bg-red-glow p-5">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-mono text-xs tracking-wide text-red">DANGER ZONE</span>
                  </div>
      ...
      <div className="px-4 py-3 flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            <div className={`w-1.5 h-1.5 ${status.color} shrink-0`} />
            <span className="text-sm font-medium text-text-primary truncate">{grant.service_provider_name}</span>
            <span className={`font-mono text-[10px] tracking-wide ${status.text}`}>{status.label}</span>
          </div>
          <div className="pl-3.5 space-y-0.5">
            {grant.capabilities.map((cap, idx) => (
              <div key={idx} className="text-xs text-text-secondary">{capabilityToHumanReadable(cap)}</div>
            ))}
          </div>
  ...
  return (
    <div className="px-4 py-3 flex items-center gap-4 bg-panel hover:bg-panel-hover transition-colors">
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
        <span className="text-sm text-text-primary">{eventLabels[event.event_type] || event.event_type}</span>
        {event.capability && (
      ...
      <span className={`font-mono text-[10px] tracking-wide ${outcome.color} shrink-0`}>
        {outcome.label}
      </span>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-surface/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="border border-red-dim bg-panel-raised max-w-md w-full p-6 animate-slide-up">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-2 h-2 bg-red" />
          <h3 className="font-mono text-sm tracking-wide text-text-primary">{title}</h3>
        </div>
