...
pub struct IssueTokenRequest {
    ...
    /// Capabilities to include in the token.
    pub capabilities: Vec<Capability>,
    /// Behavioral envelope.
    pub behavioral_envelope: BehavioralEnvelope,
    /// Optional token binding (for DPoP).
    #[serde(default)]
    ...
    let _ = state
        ...
        .record(
            AuditEvent::new(AuditEventType::TokenIssued)
                ...
                .human_principal_id(req.human_principal_id)
...
fn token_to_response(token: &AgentAccessToken, grant_id: Uuid) -> TokenResponse {
    TokenResponse {
        ...
        token_binding: None, // Token binding handled via confirmation field
    }
}
